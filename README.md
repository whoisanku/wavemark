<!--
  docs/hero.png is generated by `npm run hero` (scripts/hero.ts). Regenerate it rather than editing it.
  The src is absolute on purpose: docs/ is not in the published tarball, so a relative path would
  leave a broken image at the top of the npm package page.
-->
<p align="center">
  <img src="https://raw.githubusercontent.com/whoisanku/wavemark/main/docs/hero.png" alt="Eight wavemark avatars in a row: wave-interference patterns drawn as nodal lines in the nodal, pen and halo styles" width="720">
</p>

<p align="center"><strong>Any string becomes a wave-interference avatar. Same name, same avatar, forever.</strong></p>
<p align="center"><a href="https://whoisanku.github.io/wavemark/">Live playground</a> · <a href="https://github.com/whoisanku/wavemark/blob/main/SPEC.md">Spec</a></p>
...
The public surface, from [`src/types.ts`](https://github.com/whoisanku/wavemark/blob/main/src/types.ts) and [`src/index.ts`](https://github.com/whoisanku/wavemark/blob/main/src/index.ts):
...
| `npm run hero` | Renders `docs/hero.png` (the README hero) headlessly through the real renderer and writes the PNG with Node's zlib. Edit the name/style row in `scripts/hero.ts` and rerun; commit the PNG. |
...
docs/               hero.png, the README hero image (generated by `npm run hero`)
